...
OUTPUTS = {
  nightly: File.join(__dir__, "nightly_v2.yml"),
  release: File.join(__dir__, "release.yml")
}.freeze
...
def workflow_data(config)
...

  {
    ...
    "jobs" => prepare_jobs.merge(compile_jobs, package_jobs, "release" => release_job(config.fetch(:release)))
  }
end
...
WORKFLOWS = {
  nightly: {
    ...
    release: {
      ...
      step_name: "Update Nightly Release",
  ...
  release: {
    ...
    release: {
      ...
      step_name: "Create Tagged Release",
      name: "DiagramIDE ${{ github.ref_name }}",
...
def workflow(config)
  yaml = YAML.dump(workflow_data(config), line_width: 1000).sub(/\A---\n/, "")
...
OptionParser.new do |parser|
  ...
  parser.on("--check", "Exit nonzero if generated workflow files are stale") { check = true }
end.parse!

generated = WORKFLOWS.transform_values { |config| workflow(config) }

if check
  generated.each do |name, contents|
    output = OUTPUTS.fetch(name)
    abort "#{output} does not exist. Run: ruby .github/workflows/nightly_v2.rb" unless File.exist?(output)
    abort "#{output} is stale. Run: ruby .github/workflows/nightly_v2.rb" unless File.read(output) == contents
  end
  puts "Generated workflow files are up to date"
else
  generated.each do |name, contents|
    output = OUTPUTS.fetch(name)
